...
class Player extends FlxGroup
{
	...
	private function onShoot(timer:FlxTimer):Void
	{
		...
		projectile.x = sprite.x + 200;
		projectile.y = sprite.y + 100;

		projectiles.forEachAlive(function(curreprojectile)
		{
			if (curreprojectile.x > (FlxG.stage.stageWidth + curreprojectile.width))
			{
				curreprojectile.kill();
			}
		});
	}
